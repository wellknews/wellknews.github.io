...
const PAGES = [
  ...
  { name: 'thread/let-it-be-homework', path: '/thread/let-it-be-homework', kind: 'index' },
  ...
  { name: 'thread/not-the-cookie', path: '/thread/not-the-cookie', kind: 'index' },
]
...
const DEVICES = [
   ...
   */
  /*
   * SESSION 목록의 판에는 장치가 없다.
   *
   * 한동안 있었다 — 사진에 다가가면 그 사진이 걷히고 같은 자리에 기록의
   * 이름이 드러났다. 그 자리에서는 «목록을 읽는 방법»이 곧 장치였으므로
   * 여기서 지켰는데, 그것이 문제였다. 무엇이 있는지 알려면 서른세 번을
   * 지나가 봐야 했고, 기록과 기록 사이의 경계도 다가가기 전에는 없었다.
   * 이제 제목은 판 위의 머리에 늘 적혀 있고, 다가갔을 때 달라지는 것은
   * 그 기록이 제 색을 되찾는 정도다 — 읽는 데 필요하지 않으므로 장치가
   * 아니고, 여기서 지킬 것도 없다.
   */
  {
    kind: 'index',
