 ...
 * 이 공간의 주소.
 *
 * 화면에 적는 주소는 '/;'로 시작하고 슬래시로 끝나지 않는다. 히어로의 '/;'가
 * 바로 아래 문들의 '/;/session'과 같은 모양이어야 그 넷이 한 체계로 읽힌다.
 *
 * 바깥은 다른 규칙을 쓴다. 루트의 진입로에 적히는 것은 «문서의 주소»이고,
 * 셋 다 index.html이 있는 실제 디렉터리라 슬래시로 끝난다(/;/, /mamaboy/,
 * /ekata/). 여기 적히는 것은 «이 앱이 만드는 자리»여서 서버에 그 파일이
 * 없다. 규칙이 다른 것은 가리키는 것이 다르기 때문이고, 그래서 각자의
 * 자리에서만 지켜지면 된다.
 *
 * 딱 하나 예외가 있다. 홈으로 가는 링크의 주소(path.home)는 '/;/'다 —
 * 그쪽은 실제 문서라 슬래시 없이 부르면 301을 한 번 탄다. 화면에 적을 때는
 * BASE를 쓰고, 실제로 갈 때는 path.home을 쓴다.
 *
 * 한 번 어긋난 적이 있다. 404의 돌아가는 링크에 바깥의 규칙대로 '/;/'라고
 * 적었다. `npm run audit`의 «남의 주소 표기»가 그 뒤로 이것을 지킨다.
 */
export const BASE = '/;'
