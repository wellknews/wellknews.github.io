...
const itinerary = (
  <>
    ...
    <p>화장실도 없는 중국 카페에 가서 쉰다고 계획했다.</p>
...
const unspoken = (
  <>
    ...
    <p>언어는 직위와 계급과 처세와 사회 규범에 맞춰 얼마든지 조정된다.</p>
...
const rank = (
  <>
    ...
    <p>
      예상하지 못한 표정, 침묵, 시선, 태도, 분위기 앞에서는 직위나 계급과 별개로 흔들리거나
      반응한다.
    </p>
    <p>
      계급이 사라진다는 뜻이 아니다. 계급과 직위가 인간 사이의 모든 상호작용을 완전히 결정하지는
      못한다는 뜻이다.
    </p>
  </>
)
...
const abstraction = (
  <>
    <p>
      신 아래 모두는 평등하지만 어떤 겁쟁이들은 계속 추상적인 개념을 자기보호하고 안식과 평화에
      쓰려고 한다.
    </p>
    <p>
      그리고 그들이 하는 말에 다수의 멍청이들은 휘둘리는 꼴이, 같은 인간이라는 사실에 대해 울렁증이
      난다.
    </p>
  </>
)
...
const purpose = (
  <>
    <p>이제야 여행의 목적을 알았다.</p>
    <p>여행을 가는 것 또한 생산적이라는 것을 충분히 정리할 수 있겠다.</p>
    <p>
      나는 이성적 판단도 감정의 일환이라고 해석하는 입장이라, 그 연장선에서 여행도 생산물이 있어야
      한다고 볼 수 있다.
    </p>
  </>
)
