...
export const divideAndConquer: Session = {
  ...
  guide: (
    <>
      <p>
        Divide and Conquer는 컴퓨터 과학에서 사용하는 분할 정복이라는 문제 해결 방식의 이름이다.
      </p>

      <p>
        큰 문제를 한 번에 처리하지 않고, 여러 개의 작은 문제로 나눈 뒤 각각을 해결하고 다시
        결합한다.
      </p>

      <p>
        개발자인 나는 원래 익숙하게 알고 있던 표현이지만, 이날 처음부터 알고리즘에 대해 생각하며
        돌아다닌 것은 아니다.
      </p>

      <p>출발점은 오히려 작은 음식과 음료였다.</p>

      <p>
        오마카세에서는 한 점씩 음식이 나오고, 에스프레소 플라이트에서는 작은 여러 잔이 순서대로
        놓인다. 각각은 작지만 그 작은 단위 안에는 꽤 많은 선택과 디테일이 들어 있다.
      </p>

      <p>
        서로 관계없는 두 장소를 지나면서 이 구조가 내가 평소 문제를 처리하는 방식과도 닮아 있다는
        생각이 연결되기 시작했다.
      </p>

      <p>
        따라서 제목의 개발 용어는 단순한 말장난이라기보다 이날 서로 다른 경험을 묶어주는 비유로
        사용된다.
      </p>
    </>
  ),
