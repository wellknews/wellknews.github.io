...
export const wellknews1k: Session = {
  ...
  guide: (
    <>
      <p>WELLKNEWS는 처음에는 거의 전적으로 화면 안에서만 존재하는 프로젝트였다.</p>

      <p>로고도, 게시물도, 팔로워 숫자도, 웹페이지도 모두 디지털 화면을 통해서만 볼 수 있었다.</p>

      <p>
        팔로워가 1,000명에 도달했을 때 거창한 기념행사를 하거나 바로 상품을 판매하려고 한 것은
        아니었다. 그동안 화면 안에서 만들던 것 중 하나를 실제 물건으로 한번 꺼내보고 싶었다.
      </p>

      <p>그래서 가장 간단하게 제작할 수 있는 티셔츠를 골랐다.</p>

      <p>
        이 페이지에서 처음에는 디지털 시안이 보이고 스크롤이 진행되면서 실제 물건으로 넘어가는 것도
        그 순서를 그대로 보여주기 위한 구성이다.
      </p>

      <p>
        중요한 것은 티셔츠 자체의 상품성보다 화면 속에서 만들던 것이 생각보다 쉽게 손으로 만질 수
        있는 물건이 되었다는 경험이었다.
      </p>

      <p>1,000이라는 숫자는 그 실험을 시작하게 만든 계기다.</p>
    </>
  ),
  display: 'stage',
   ...
   *   · 움직임의 폭을 줄인다. 없애는 것이 아니다 — 만질 수 있는 것은 좁은
   *     화면에서도 전부 만져지고, 다만 같은 거리로 밀리면 넓은 화면에서
   *     «공간의 온도»였던 것이 여기서는 화면 전체가 흔들리는 일이 되므로
   *     그 크기만 낮춘다. 무엇이 얼마나 줄어드는지는 `layout/viewport.ts`에 있다.
   *
  ...
  compact: (
    <>
      ...
      {/* 넓은 화면과 같은 판이다. 손가락은 톡 누르거나 좌우로 밀어 뒤집는다. */}
      <Scene width="bleed">
        <Faces front={mockupFront} back={mockupBack} />
      </Scene>

      {/* 이 배치에서 가장 크게 일어나는 일. 앞뒤로 여백을 더 준다. */}
      <Scene width="bleed" air>
        <Becoming from={mockupFront} to={packaged} />
      </Scene>
