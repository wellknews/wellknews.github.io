...
export type Cover = {
   ...
   * 배경을 딴 이미지인지.
   *
   * 초점과 같은 종류의 사실이다 — 연출이 아니라 이 파일이 어떻게 생겼는지다.
   * 배경이 투명한 이미지는 장면이 아니라 «놓인 물건»이라서, 장면을 자르듯
   * 자르면 남는 것이 없다. 시안 목업이 그랬다. 판 위의 조각에서 목과 밑단이
   * 한꺼번에 잘려 나가, 사진을 채운 것이 아니라 셔츠를 토막 낸 것이 되었다.
   *
