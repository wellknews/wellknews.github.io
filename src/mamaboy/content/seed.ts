...
const entries: Seed[] = [
  ...
  },
  {
    ...
    bodyOriginal: [
      {
        kind: 'text',
        text: '리메이크가 팔리는 이유를 향수로 설명하면 그다음 문장이 없다. 향수는 감정의 이름이지 구매의 이유가 아니다.',
      },
      {
        kind: 'text',
        text: '다시 사는 사람들이 되찾으려는 것은 그 시절이 아니라 그 시절에 가능했던 밀도다. 한 가지를 오래 들여다보는 일, 끝까지 가보는 일.',
      },
      /* 글 사이에 낀 그림. 전문을 가져올 때 이 자리가 살아 있는지 화면에서 확인하려고 둔다. */
      {
        kind: 'image',
        url: 'https://example.com/skin-ledger/remake-shelf.jpg',
        alt: '같은 완구의 1998년판과 2026년 재발매판이 나란히 놓인 선반',
      },
      {
        kind: 'text',
        text: '그래서 같은 물건이라도 «그때 그대로»를 파는 쪽보다 «지금 만들었다면 이랬을 것»을 파는 쪽이 더 오래 남는다.',
      },
    ],
  },
  {
